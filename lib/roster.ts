...
For each patient, choose exactly one provider and schedule a specific time slot for that day's visit, in "H:MM AM/PM" format (e.g. "2:00 PM" to "3:00 PM").
...
For each assignment, DO NOT compute an overall weighted score yourself — that is calculated separately from the raw factor scores you provide. Instead, score the chosen provider against EACH of these six factors independently, each on a 0-100 scale, judged only on how well that specific factor is satisfied (ignore how important the factor is — that weighting happens elsewhere):
...
Also give a one-sentence reasoning citing which factors most influenced the choice, given these raw factor scores. Also produce a brief overall summary (1-2 sentences) of the day's roster.`;
}
